    ...
    [](const myPointer &readings, const std::list<myVariant> &sensorIds)
{
    ...
    float total = 0;
    int count = 0;
    for (auto &itr : sensorIds)
    { // sensor id mein loop lagao
...
std::function<std::optional<myPointer>(const myPointer &, int)> findFirstNSensors =
    [](const myPointer &readings, int N)
{
    if (N > readings.size() || N < 0)
    {
        throw std::runtime_error("Invalid Value");
    }

    auto itr = std::next(readings.begin(), N);
    myPointer result(readings.begin(), itr);
